"""
Test subprocess output capture WITH the wrapper
This tests the actual production setup
"""
import subprocess
import sys
import os
...
print("Testing subprocess output capture WITH WRAPPER...")
print("=" * 50)
...
# Get wrapper path
wrapper_path = os.path.join("app", "tasks", "script_wrapper.py")

# Run the script THROUGH WRAPPER (like production)
process = subprocess.Popen(
    [sys.executable, "-u", wrapper_path, script_path],
...
if len(lines_captured) == 5:
    print("✓ SUCCESS: Wrapper works correctly!")
else:
    print("✗ PROBLEM: Wrapper is losing output!")
    print("\nThis means the issue is in script_wrapper.py")
